...
class enhance_net_nopool_shiftrgb(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(x - b1) + r1 * (torch.pow(x, 2) - x)
        x = self.relu(x - b2) + r2 * (torch.pow(x, 2) - x)
        x = self.relu(x - b3) + r3 * (torch.pow(x, 2) - x)
        enhance_image_1 = self.relu(x - b4) + r4 * (torch.pow(x, 2) - x)
        x = self.relu(enhance_image_1 - b5) + r5 * (torch.pow(enhance_image_1, 2) - enhance_image_1)
        x = self.relu(x - b6) + r6 * (torch.pow(x, 2) - x)
        x = self.relu(x - b7) + r7 * (torch.pow(x, 2) - x)
        enhance_image = self.relu(x - b8) + r8 * (torch.pow(x, 2) - x)
        r = torch.cat([r1, r2, r3, r4, r5, r6, r7, r8], 1)
